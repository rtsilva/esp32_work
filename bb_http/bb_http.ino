...
void loop() {
  ...
  while (wm.run() == WL_CONNECTED) { //&& clock()-currentTime >= 2000){
    ...
    if (!client.connect("iesc-s3.mit.edu", 80)) {
      ...
      client.println("GET http://iesc-s3.mit.edu/breadboard/deviceInquiry?dev_id=231 HTTP/1.1");
    ...
    if (op.indexOf("No") != -1) {
      ...
      */
      ...
      if (response.length() > 0) {
        ...
        String dict = "{\"dev_id\": \"231\", \"task_id\": \""+task_id+"\", \"values\": \""+response+"\"}";
